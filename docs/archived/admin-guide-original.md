...
This guide provides comprehensive information for system administrators deploying and maintaining the Image Project Manager application.
...
Image Project Manager is a full-stack web application for managing, classifying, and collaborating on visual content. It consists of:
...
# Save logs to file
docker logs image-project-manager > app.log 2>&1
```
...
docker logs image-project-manager | grep -i auth
```
...
# Check backend logs
docker logs image-project-manager | grep -i s3
```
...
```bash
# Start new version
docker run -d --name image-manager-new -p 8001:8000 ...
