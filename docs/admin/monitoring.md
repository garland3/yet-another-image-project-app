...
This guide covers monitoring, logging, health checks, and regular maintenance for the Image Project Manager.
...
docker stats image-project-manager
```
...
# Last 100 lines
docker logs --tail 100 image-project-manager

# Logs from last hour
docker logs --since 1h image-project-manager

# Save logs to file
docker logs image-project-manager > /var/log/docker-app.log 2>&1
```
...
kubectl logs --previous deployment/image-project-manager

# Logs from specific container
kubectl logs deployment/image-project-manager -c app

# Save logs
kubectl logs deployment/image-project-manager > app.log
```
...
/var/log/image-manager/*.log {
    ...
    sharedscripts
    postrotate
        docker kill -s HUP image-project-manager 2>/dev/null || true
    endscript
}
```
...
docker run -d --name app-green -p 8001:8000 image-project-manager:latest
...
kubectl set image deployment/image-project-manager \
  app=image-project-manager:v1.1.0

# Watch rollout
kubectl rollout status deployment/image-project-manager

# Rollback if needed
kubectl rollout undo deployment/image-project-manager
```
