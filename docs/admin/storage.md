...
This guide covers S3-compatible object storage setup and management for the Image Project Manager.
...
```bash
# Create bucket
mc mb myminio/image-project-manager

# Set public download policy (for presigned URLs)
mc policy set download myminio/image-project-manager

# Or more restrictive policy
mc policy set-json myminio/image-project-manager <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
      ...
      "Principal": {"AWS": ["*"]},
      "Action": ["s3:GetObject"],
      "Resource": ["arn:aws:s3:::image-project-manager/*"],
      "Condition": {
        "StringLike": {
...
```bash
# Using AWS CLI
aws s3 mb s3://image-project-manager --region us-east-1

# Enable versioning (optional)
aws s3api put-bucket-versioning \
  --bucket image-project-manager \
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:s3:::image-project-manager",
        "arn:aws:s3:::image-project-manager/*"
      ]
    }
  ]
...
**MinIO:**
```bash
mc version enable myminio/image-project-manager
```
...
aws s3 mb s3://image-project-manager-replica --region us-west-2
...
aws s3 sync s3://image-project-manager s3://backup-bucket --storage-class GLACIER

# Or use rclone for multi-cloud backup
rclone sync minio:image-project-manager b2:backup-bucket
```
...
aws cloudwatch get-metric-statistics \
  ...
  --dimensions Name=BucketName,Value=image-project-manager \
  --statistics Average \
...
aws cloudfront create-distribution \
  --origin-domain-name image-project-manager.s3.amazonaws.com \
...
S3_ENDPOINT=image-project-manager.s3-accelerate.amazonaws.com
```
...
MinIO:
```bash
mc encrypt set sse-s3 myminio/image-project-manager
```
...
aws s3api copy-object \
  --bucket image-project-manager \
  --copy-source image-project-manager/projects/abc-123/image.jpg?versionId=VERSION_ID \
...
aws s3 sync s3://backup-bucket s3://image-project-manager
```
...
# Test with curl
curl -v http://localhost:9000/image-project-manager/
```
...
aws s3 cp test.txt s3://image-project-manager/test.txt
```
