...
This guide covers common issues and their solutions for the Image Project Manager.
...
docker ps | grep image-project-manager

# Check logs
docker logs --tail 50 image-project-manager

# Database connection
psql -h localhost -U postgres -d imagemanager -c "SELECT 1;"

# S3 connection
aws s3 ls s3://image-project-manager/ --endpoint-url http://localhost:9000
```
...
docker ps | grep image-project-manager || echo "Container not running!"
echo
...
echo "4. Storage:"
mc du myminio/image-project-manager 2>&1 || echo "MinIO check failed"
echo
...
docker logs --since 1h image-project-manager 2>&1 | grep -i error | tail -10
```
...
2. **Port already in use:**
   ...
   docker run -p 8001:8000 image-project-manager
   ```
...
1. **Development mode not enabled:**
   ...
   # Restart application
   docker restart image-project-manager
   ```
...
docker logs image-project-manager | grep -i "403\|forbidden\|authorization"
```
...
mc ls myminio/image-project-manager/projects/
aws s3 ls s3://image-project-manager/projects/
...
```bash
# Check upload logs
docker logs image-project-manager | grep -i "upload\|POST /api/images"
...
# Check S3 storage
mc du myminio/image-project-manager
```
...
```bash
# Check memory usage
docker stats image-project-manager --no-stream

# Check for memory leaks in logs
docker logs image-project-manager | grep -i "memory\|oom"

# Monitor Python process
top -p $(docker inspect -f '{{.State.Pid}}' image-project-manager)
```
...
1. **Increase memory limit:**
   ```bash
   # Docker
   docker run -d --memory="4g" image-project-manager
...
2. **Optimize cache settings:**
   ...
   # Clear cache
   docker exec image-project-manager rm -rf /tmp/cache/*
   ```

3. **Restart application periodically:**
   ```bash
   # Add to crontab (daily restart at 3 AM)
   0 3 * * * docker restart image-project-manager
   ```
...
```bash
# Check exit code
docker inspect image-project-manager | jq '.[0].State'

# View crash logs
docker logs image-project-manager

# Check events
docker events --filter container=image-project-manager
...
1. **Out of memory:**
   ...
   docker run -d --memory="4g" --memory-swap="4g" image-project-manager
   ```

2. **Uncaught exceptions:**
   ```bash
   # Check logs for stack traces
   docker logs image-project-manager | grep -A 20 "Traceback"
...
# View detailed logs
docker logs -f image-project-manager
```
...
docker logs --tail 100 image-project-manager >> $REPORT_FILE 2>&1
echo >> $REPORT_FILE
