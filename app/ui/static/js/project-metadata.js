 ...
 * Handles CRUD operations for project metadata
 */
function projectMetadataManager() {
  return {
    metadata: [],
    ...
    async loadMetadata() {
      try {
        ...
        showError('Failed to load project metadata. Please try again later.');
      }
    },
    ...
    async updateMetadata() {
      try {
        const projectId = new URLSearchParams(window.location.search).get('id');
        if (!projectId || !this.editingMetadata) return;
        
        const response = await fetch(`/projects/${projectId}/metadata/${this.editingMetadata.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
          },
    ...
    async deleteMetadata(id) {
      ...
      try {
        ...
        const response = await fetch(`/projects/${projectId}/metadata/${id}`, {
          method: 'DELETE'
        });
